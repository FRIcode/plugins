...
public class Camera {
    ...
    private Boolean focusPreCaptureLock = false;
    ...
    private void createCaptureSession(
        ...
        flutterSurface = new Surface(surfaceTexture);
    ...
    public void takePicture(@NonNull final Result result) {
        ...
        if (useAutoFocus && focusPreCaptureLock) {
            lockFocus();
        } else {
            runPictureCapture();
        }
...
//    Log.i("flutter", "runPictureCapture");
        ...
        try {
            captureSurface = pictureImageReader.getSurface();
            captureRequestBuilder.removeTarget(flutterSurface);
            captureRequestBuilder.addTarget(captureSurface);
            cameraCaptureSession.stopRepeating();
            cameraCaptureSession.capture(captureRequestBuilder.build(), mCaptureCallback, null);
        } catch (CameraAccessException e) {
    ...
    private void updateFocus(FocusMode mode) {
        if (useAutoFocus) {
            ...
            if (modes == null
                ...
                focusPreCaptureLock = mode != FocusMode.autoNoLock;
